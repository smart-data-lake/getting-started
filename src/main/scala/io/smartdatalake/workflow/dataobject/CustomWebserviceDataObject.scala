...
// This is the CustomWebserviceDataObject implementation as it should look after completing part-3
...
case class CustomWebserviceDataObject(override val id: DataObjectId,
  ...
  private val sparkResponseRowSchema = responseRowSchema.asInstanceOf[SparkSchema].inner
  ...
  override def getSparkDataFrame(partitionValues: Seq[PartitionValues])(implicit context: ActionPipelineContext): DataFrame = {
    ...
    if (context.phase == ExecutionPhase.Init) {
      ...
      val df = Seq[String]().toDF("responseString")
      ...
      val currentQueryParameters = if (previousState.isEmpty) checkQueryParameters(queryParameters)
      else checkQueryParameters(previousState.map {
        x => DepartureQueryParameters(x.airport, x.nextBegin, x.nextBegin + now -60) // current minute not always supported by API...
      })
      ...
      val departuresDf = if (mockJsonDataObject.nonEmpty) {
        ...
        val mockDf = mockDeparturesDf
          .withColumn("firstSeen", $"firstSeen" - minLastSeen + paramBegin)
